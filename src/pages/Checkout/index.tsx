...
const Checkout = () => {
  ...
  const handleInputChange = <T extends keyof FormValues>(name: T, value: FormValues[T]) => {
  ...
  const options = {
    clientId: 'AcfPEz69Itpd35FRcq0KdcZzxDUVX_m3REkCgoIwoG9JojUOymHtMl-Og8wHlWPTK1JxWhWqKSgIhcER',
  };
  ...
  const handleCheckout = async () => {
    ...
    const tokenResponse = await fetch('https://api.sandbox.paypal.com/v1/oauth2/token', {
        ...
        headers: {
          ...
          'Authorization': 'Basic ' + btoa('AcfPEz69Itpd35FRcq0KdcZzxDUVX_m3REkCgoIwoG9JojUOymHtMl-Og8wHlWPTK1JxWhWqKSgIhcER:8mrZ#J#8'), // Replace with your actual client ID and secret
        },
    ...
    if (result.links && Array.isArray(result.links)) {
        ...
        const approveLink = result.links.find((link: PayPalLink) => link.rel === 'approve');
        ...
        if (approveLink) {
          ...
          console.error('Error creating order: No "approve" link found in the response');
          // Handle the error as needed
        }
      } else {
        console.error('Error creating order: Invalid response structure');
        // Handle the error as needed
      }
    } catch (error) {
    ...
    <PayPalScriptProvider options={options}>
        ...
        <div className="main-container w-100">
            ...
            <form>
                ...
                <div className="form-group">
                    <h3>Estimated Delivery Date: {formData.deliveryDate}</h3>
                </div>
            </form>
            </div>
            ...
            <p>Proceed to pay</p>
            <PayPalButtons style={{ layout: 'horizontal' }} onClick={handleCheckout} onApprove={onApprove} onError={onError}/>
            </div>
        </div>
        </div>
